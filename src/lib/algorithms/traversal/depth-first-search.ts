...
import { isInStack, getUntraversedNeighbours, isEqualTile } from "../../../utils/helper-functions";

// function to execute the depth-first-search traversal algorithm for visualization
export const dfs = (
    grid: GridType,
    startTile: TileType,
    endTile: TileType
    ...
    while (untraversed.length) {
        // get the first tile from the stack
        ...
        if (tile) {
            ...
            for (let i = 0; i < untraversedNeighbours.length; i++) {
                // check if the neighbour is not in the stack
                if (!isInStack(untraversedNeighbours[i], untraversed)) {
                    ...
                    // add the neighbour to untraversed stack
